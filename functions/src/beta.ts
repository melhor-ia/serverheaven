...
const notificationEmailTemplate = (unsubscribeLink: string) => `
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
export const betaSignup = functions.https.onCall({ secrets: [GMAIL_PASSWORD] }, async (request: functions.https.CallableRequest) => {
  ...
  if (!choice || (choice !== "feedback" && choice !== "notify")) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "A valid choice ('feedback' or 'notify') is required."
    );
  }
 ...
 */
export const verifyBetaToken = functions.https.onCall(async (request) => {
 ...
 */
export const completeBetaProfile = functions.https.onCall(async (request) => {
