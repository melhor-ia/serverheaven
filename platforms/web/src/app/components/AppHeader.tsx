            ...
            <div className="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-20">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Link href={`/profile/${mockUser.username}`} passHref>
                            <div className="flex items-center gap-3 group cursor-pointer p-2 rounded-lg hover:bg-white/5 transition-colors">
                                <img src={mockUser.avatar_url} alt="User Avatar" className="h-10 w-10 rounded-full border-2 border-border group-hover:border-emerald-500 transition-colors" />
                                <div className="text-left hidden lg:block">
                                    <p className="font-bold text-white group-hover:text-emerald-400 transition-colors">{mockUser.displayName}</p>
                                    <p className="text-xs font-mono text-muted-foreground">@{mockUser.username}</p>
                                </div>
                            </div>
                        </Link>
                         <Button variant="ghost" size="icon" className="group">
